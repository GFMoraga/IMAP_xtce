...
# Create the CCSDSPacket SequenceContainer (as before)
ccsds_packet_container = ET.SubElement(container_set, "{http://www.omg.org/space}SequenceContainer")
ccsds_packet_container.attrib["name"] = "CCSDSPacket"
ccsds_packet_entry_list = ET.SubElement(ccsds_packet_container, "{http://www.omg.org/space}EntryList")

# Add ParameterRefEntry elements for CCSDS parameters (as before)
ccsds_parameters = ["VERSION", "TYPE", "SEC_HDR_FLG", "PKT_APID", "SEG_FLGS", "SRC_SEQ_CTR", "PKT_LEN"]
for param_name in ccsds_parameters:
    param_ref_entry = ET.SubElement(ccsds_packet_entry_list, "{http://www.omg.org/space}ParameterRefEntry")
    param_ref_entry.attrib["parameterRef"] = param_name

# Create the CoDICESciencePacket SequenceContainer (with condition)
codice_science_container = ET.SubElement(container_set, "{http://www.omg.org/space}SequenceContainer")
codice_science_container.attrib["name"] = "CoDICESciencePacket"
codice_science_base_container = ET.SubElement(codice_science_container, "{http://www.omg.org/space}BaseContainer")
codice_science_base_container.attrib["containerRef"] = "CCSDSPacket"
codice_science_restriction = ET.SubElement(codice_science_base_container, "{http://www.omg.org/space}RestrictionCriteria")
comparison = ET.SubElement(codice_science_restriction, "{http://www.omg.org/space}Comparison")
...
# Create the P_COD_AUT SequenceContainer (as before)
pcod_aut_container = ET.SubElement(container_set, "{http://www.omg.org/space}SequenceContainer")
pcod_aut_container.attrib["name"] = "P_COD_AUT"
pcod_aut_entry_list = ET.SubElement(pcod_aut_container, "{http://www.omg.org/space}EntryList")

# Add ParameterRefEntry elements for other parameters in P_COD_AUT (as before)
for index, row in df.iterrows():
    if index < 8:
        continue  # Skip rows before row 9

    param_name = row['mnemonic']
    param_ref_entry = ET.SubElement(pcod_aut_entry_list, "{http://www.omg.org/space}ParameterRefEntry")
    param_ref_entry.attrib["parameterRef"] = param_name
...
tree.write("p_cod_aut_def.xml", encoding="utf-8", xml_declaration=True)